...
// TODO: 尝试更改为Mixin写法
public class InfiniteWaterBucket extends Item implements FluidModificationItem {
    private final Fluid fluid;

    public InfiniteWaterBucket(Item.Settings settings) {
        super(settings.rarity(Rarity.EPIC).maxCount(1));
        this.fluid = Fluids.WATER;
    }

    public static ItemStack getEmptiedStack(ItemStack stack, PlayerEntity player) {
        return stack;            // Infinity Here: 直接返回原 ItemStack
    }
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        tooltip.add(Text.translatable("tooltip.weird_items.infinite_water_bucket.desc").formatted(Formatting.GRAY));
    }
}
